...
exports.register = async (req, res, next) => {
  ...
  // if user is not verified than update prev one
    ...
    req.userId = new_user._id; // this userId can be accessed in the next middleware which is used  in the /register route (first one was authcontroller.register and the second one)
    next();
  }
};
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) If token has not expired, and there is user, set the new password and update passwordChangedAt property for the user
  if (!user) {
    return res.status(400).json({
      status: "error",
  ...
  // 3) Update changedPasswordAt property for the user DONE
