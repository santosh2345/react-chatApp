...
const http = require("http"); // it is going to come from the node js alone and no need to npm i http if machine has already nodejs install
const User = require("./models/user");
const FriendRequest = require("./models/friendRequest");
const server = http.createServer(app); // now server is just created successfully using app and http.createserver
...
const DB = process.env.DBURI.replace("<PASSWORD>", DBPASS);
...
//       // data = {to, from }
//       const { to, from } = data;
//       // check if there's any existing conversations between these two users
...
//       //if there's existing_conversation
...
//       // data: {to , from , message, conversation_id, type ={doc, link, text ....}}
...
//       // create a conversation only if it doesn't exist yet or add new message to the messages list
//       const chat = await OneToOneMessage.findById(conversation_id);
//       chat.messages.push(new_message);
